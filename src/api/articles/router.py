...
router = APIRouter(tags=["API Articles"])

@router.get("/articles", response_description="List all articles", response_model=ArticleCollection)
async def list_articles_api():
    return ArticleCollection(articles=await blog_collection.find().to_list(10))

@router.post("/articles", response_model=ArticleModel, response_description="Add new article")
...
@router.get("/articles/{id}", response_description="Get a single article", response_model=ArticleModel, response_model_by_alias=False)
...
@router.patch("/articles/{id}", response_description="Update a article", response_model=ArticleModel, response_model_by_alias=False)
...
@router.delete("/articles/{id}", response_description="Delete a article")
